...
class TXTtoXMLHandler:
    ...
    def txt_to_xml(self):
        ...
        # the common fomat of Element Node <addr> as well as its subElement
        # Node <org> and <detail>
        _node = """\t<addr id="%s">
			<org>%s</org>
			<detail>%s</detail>\n\t</addr>"""
        ...
        while True:
            ...
            # when it comes to EOF, an empty string will be returned by
            # readline()
            if not line:
                break
            else:
                # split the first with the delimeter "(hex)"
                _IdOrg = map(self._stripEnds, line.split("(hex)"))

                self._fileTXT.readline()  # the 2nd line will be omitted

                # read the following 3 to 4 lines which the detail info of
                # manufacturers will be constructed
                _info = []
                line = self._fileTXT.readline()
                while line != "\n":
                    _info.append(line)
                ...
                _nodeTMP = _node % (self._handleEntityRef(_IdOrg[0]), self._handleEntityRef(
                    _IdOrg[1]), self._handleEntityRef(_detail))
