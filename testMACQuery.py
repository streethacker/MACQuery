...
class BadInputMAC(unittest.TestCase):
	def testBadMAC(self):
		for addr in ("FC-F6", "E8-9A-8F-FC-78-C9-A0", "20-59-Z0", "20-59-A0-", \
				"9C-06-6K", "*9-06-7E", "5$-22-33", "9C.3A.AF", "E4&2A&D3"):
				self.assertRaises(MACQuery.BadFormatError, handler.query, addr)

	def testUnknownMAC(self):
		for addr in ("88-99-10", "AA-BB-CC", "FF-AE-00", "cb-09-9d", \
				"2b-ce-40", "7A-22-F5", "F1-F5-5B", "E1-F5-CA", "D8-B8-F5"):
