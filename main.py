...
(default to local machine's MAC addrs):\n").split(",")
    ...
    def _getBy(self, *argv):
        for addr in argv:
            try:
                ...
                _tmp_warning_bad_format = self._warning_bad_format % (
                    addr, str(err))
                self.result.append(_tmp_warning_bad_format)
            except BadRequestError as err:
                _tmp_warning_not_found = self._warning_not_found % (
                    addr, str(err))
                self.result.append(_tmp_warning_not_found)
            else:
                _tmp_resultInfo = self._resultInfo % (
                    _result["id"], _result["org"], _result["detail"])
